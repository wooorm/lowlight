...
function registerAlias(aliasMapOrLanguageName, aliasListOrName) {
  if (
    !aliasListOrName &&
    aliasMapOrLanguageName.toString() !== aliasMapOrLanguageName
  ) {
    Object.keys(aliasMapOrLanguageName).forEach(function(key) {
      registerAlias(key, aliasMapOrLanguageName[key])
    })
    return
  }
  if (
    Array.isArray(aliasListOrName) &&
    aliasMapOrLanguageName.toString() === aliasMapOrLanguageName
  ) {
    aliasListOrName
      ...
      .forEach(function(element) {
        aliases[element] = aliasMapOrLanguageName
      })
    return
  }
  if (
    ...
    registerAlias(aliasMapOrLanguageName, [aliasListOrName])
  }
}
